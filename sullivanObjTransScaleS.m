...
for k=1:129
    for j=1:65
        for i=1:65
			xyz_new(:,i,j,k) = s_tt *(qfT*xyz(:,i,j,k)-qfT*bvcf1);
        end
    end
end
%%
...
fid = fopen('sullivan_newobjScal.xyz','w');
    ...
    for j=1:65
        for i=1:65
          %  c1=dqfT*xyz(:,i,j,k)-dqfT*bvcf1-qfT*dbvf1;
            c1=(s_tt*dqfT+qfT)*xyz(:,i,j,k)-s_tt*dqfT*bvcf1-s_tt*qfT*dbvf1 ...
			- qfT*bvcf1;
			sol_new(:,i,j,k) = s_tt*qfT*sol(:,i,j,k)+c1;
        end
    end
end
...
fid = fopen('sullivan_newobjScal.q','w');
...
p88=1/s_tt*qfT'*p88_new+bvcf1;
p1_new = s_tt*(qfT*p1-qfT*bvcf1);
p2_new = s_tt*(qfT*p2-qfT*bvcf1);
p3_new =  s_tt*(qfT*p3-qfT*bvcf1);
p4_new = s_tt*(qfT*p4-qfT*bvcf1);
p5_new = s_tt*(qfT*p5-qfT*bvcf1);
p6_new = s_tt*(qfT*p6-qfT*bvcf1);
p7_new = s_tt* (qfT*p7-qfT*bvcf1);
p8_new = s_tt* (qfT*p8-qfT*bvcf1);
